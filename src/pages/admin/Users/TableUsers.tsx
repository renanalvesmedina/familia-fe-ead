      ...
      {users == null || users == undefined ? (
        ...
        <table className="table table-zebra w-full">
          ...
          <thead className="border-b w-full">
            <tr className="w-full">
              <th className="rounded-none shadow sticky top-0 after:absolute bg-white text-sm">
                Nome
              </th>
              <th className="shadow sticky top-0 after:absolute bg-white text-sm">
                Email
              </th>
              <th className="shadow sticky top-0 after:absolute bg-white text-sm">
                Perfil
          ...
          <tbody>
            {users?.map((user) => (
              <tr key={user.userId}>
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="avatar">
                      <div className="mask mask-circle w-8 h-8">
                        <img
                          src={
                            user.photoUri == undefined ? avatar : user?.photoUri
                          }
                          alt={user.fullName}
                        />
                      </div>
                ...
                <th>
                  <button
                    onClick={() => _navigate(`/admin/user/edit/${user.userId}`)}
                    className="bg-zinc-900 px-2 py-[2px] rounded-lg text-sm text-white flex gap-1 items-center hover:bg-gradient-to-r from-zinc-700 active:bg-zinc-700"
                  >
                    <NotePencil size={16} />
                    Editar
                  </button>
